...
def lerp_held(held_item: HeldItem, time: float=1, position: glm.vec3=None, rotation: glm.quat=None, end_func: Callable=None, right: bool=False) -> Callable:
    ...
    def func(dt: float) -> None:
        held_item.percent_moved = glm.clamp(held_item.percent_moved + dt / time * (2 * (game.mouse.left_down if not right else game.mouse.right_down) - 1), 0, 1)
